    ...
    for ci,c in enumerate(cols_iter):
        ...
        with open(outputfile,'w') as o:
            ...
            for g in ge:
                ...
                ge_df = ge_df.drop_duplicates('sgRNA Sequence')
                gene = g
                sgrnas = list(ge_df['sgRNA Sequence'])
                sgrna_list = ';'.join(sgrnas)
                length_sps = len(sgrnas)
                rank = list()
                for s in sgrnas:
                    r=sp_rank[s]
                    rank.append(r)
                ...
                if len(indices) == 0:
                    ...
                    sgrna_rank = sc_array.index(min(sc_array))+index_add
                    log_score = -log10(min(sc_array))
                    avg = (avg + sum([-log10(x) for x in sc_array]))/(len(indices)+1)
                    #sp_stars.append(log_score)
                    w.writerow((gene, length_sps, all_ranks, sgrna_list, sgrna_rank, log_score, avg))
